...
Endform analyzes your suite and uploads each test to its own isolated cloud runner.
Completed test results are both sent back to the CLI for local inspection, and to Endform to view later and track suite health in the dashboard.
...
- **Configuration with `endform.config.ts`**: Tune file transfer, environment variables, concurrency limits, regions, network proxying, reporters, and trace retention.
