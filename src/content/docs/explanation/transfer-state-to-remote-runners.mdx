...
- All environment variables that are set in your `playwright.config.ts` are transferred - using a package like [dotenv](https://www.npmjs.com/package/dotenv) in your playwright config is supported, and those environment variables are picked up.
