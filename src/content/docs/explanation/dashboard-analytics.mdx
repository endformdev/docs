...
For example, `testRun.outcome = fail` still includes quarantined failures. When investigating a quarantined test, use its individual outcomes and retry history to judge whether it has recovered; a passing suite alone does not show that the test is healthy.
