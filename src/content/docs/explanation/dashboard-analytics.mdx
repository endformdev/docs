...
A suite can pass even when it contains failed quarantined tests. Quarantine normally prevents those failures from failing the suite, while Endform preserves each test's actual outcome, attempts, and metrics.

Quarantined tests have a **Quarantined** badge in test run tables and on the test run page. Select the badge to view the rule that applied to that run. The association remains visible after the rule expires or is deactivated.

For example, `testRun.outcome = fail` still includes quarantined failures. Quarantined tests run once without retries, so use their outcomes across subsequent suite runs to judge whether they have recovered; a passing suite alone does not show that a test is healthy.

See [Quarantine tests](/docs/guides/quarantine-tests) to create, expire, or deactivate rules and understand which failures still block a suite.
...
## Query the same data outside the dashboard

Use the [`endform analytics` and `endform samples` commands](/docs/reference/analytics-cli) to query historical test data from a terminal, script, or CI job.

You can also connect the [Endform MCP server](/docs/reference/mcp-server) to an AI coding agent and ask it to investigate your test history. See [Investigate test history with AI](/docs/guides/investigate-test-history-with-ai) for example conversations.
