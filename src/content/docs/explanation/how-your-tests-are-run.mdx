...
Here's a brief explanation of how our test runner works.
...
- The set of tests to run, and the dependencies required for each test, are then sent to our test runner service.

<Aside>
To see what code and environment variables endform detects, run `npx endform test --debug` for verbose output that includes this information.
</Aside>
...
- Each machine only ever runs one attempt of a test. The test runner can immediately schedule _more than one_ retry of a test when the first run fails.
