...
1. ### Configure which traffic to proxy.
   ...
   If you're running a server outside of Playwright, that's fine. You can tell Endform which traffic to proxy by using an `endform.jsonc` file.
   ...
   You can configure both at the same time if needed. You can read more about `proxyNetworkHosts` and `proxyNetworkPorts` in the [endform config reference](/docs/reference/endform-config#proxy-network-hosts).
