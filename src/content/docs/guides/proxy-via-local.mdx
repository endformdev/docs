...
description: How to proxy traffic from your remote runners to your local network. This helps you run tests against local servers
sidebar:
  order: 3
---
...
<Aside>
`proxyNetworkHosts` is the easiest option for most setups. `proxyNetworkPorts` is available for raw TCP connections like databases.
</Aside>
...
1. ### Configure which traffic to proxy.
   ...
   If you're running a server outside of Playwright, that's fine. You can tell Endform which traffic to proxy by using an `endform.config.ts` file.
   ...
   <Code language="ts" title="endform.config.ts" code={`
   import { defineEndformConfig } from "endform";

   export default defineEndformConfig({
     proxyNetworkHosts: ["<loopback>", "*.my-special-domain.com"],
   });
   `} />

   If you need raw TCP connections to a local loopback port, use `proxyNetworkPorts` instead. Useful for things like database connections.
   ...
   You can configure both at the same time if needed. You can read more about `proxyNetworkHosts` and `proxyNetworkPorts` in the [endform config reference](/docs/reference/endform-config-ts#proxynetworkhosts).
