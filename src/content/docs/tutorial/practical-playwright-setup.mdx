...
<Aside>
  To start from this step, check out the [`stage-0-baseline`
  branch](https://github.com/endformdev/playwright-tutorial/tree/stage-0-baseline)
  of the [tutorial
  repository](https://github.com/endformdev/playwright-tutorial). For more
  information about getting started with the tutorial repository, go
  [here](/docs/tutorial/playwright-e2e-from-cowboy-to-confidence#getting-started-with-the-tutorial-repository).
</Aside>
...
Follow the wizard. You should now have a `playwright.config.ts` file and a `tests` directory.
...
<Aside type="caution">
  When you run Playwright in debug mode for the first time, the browser will
  open and appear frozen on `about:blank`. This is expected! You need to click
  the "step over" button in the Playwright Inspector window to start stepping
  through your test.
</Aside>
...
<Image
  src={StepOverDebug}
  alt="Step over button in Playwright's inspector"
  width={600}
/>
...
<Tabs>
  <TabItem label="Linux/macOS">
    ```bash touch tests/setup.spec.ts tests/teardown.spec.ts ```
  </TabItem>
  <TabItem label="Windows (PowerShell)">
    ```powershell New-Item -ItemType File -Path tests/setup.spec.ts,
...
<Aside type="tip">
The code below only shows the `storageState` additions. Keep your existing project configurations (like `testMatch`, `testIgnore`, `dependencies`, and device settings) and just add the `storageState` to the `use` object.
</Aside>
...
export default defineConfig({
	projects: [
		...
		},
		{
			name: "chromium",
			use: {
				// Add this to your existing use configuration
...
```bash
git checkout origin/stage-1-setup -- app/api/internal/user/route.ts
```

<Aside type="note">
  This step is only necessary if you're running tests against a local Next.js
  dev application. If you're testing against the remote Vercel deployment, this
  API endpoint is already available and you can skip this checkout command.
</Aside>
...
<Tabs>
  <TabItem label="Linux/macOS">
    ```bash touch global-setup.ts global-teardown.ts ```
  </TabItem>
  <TabItem label="Windows (PowerShell)">
    ```powershell New-Item -ItemType File -Path global-setup.ts,
    global-teardown.ts ```
  </TabItem>
</Tabs>
...
<Aside type="tip">
  Update the `storageState` path in your existing project configurations. Keep
  all your other settings (like `testMatch`, `testIgnore`, `dependencies`, and
  ...
  projects: [
    {
      name: "chromium",
      use: {
        // Change the storageState path to use the API-created user
