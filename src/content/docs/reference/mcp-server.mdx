...
Lists the organizations available to the active Endform login. It takes no arguments. An agent can use the returned organization ID in the other historical data tools.
...
<ParameterTable>
  ...
  | --- | --- | --- |
  | `organizationId` | string | Organization to query. |
...
<ParameterTable>
  ...
  | Parameter | Type | Description |
  | --- | --- | --- |
  | `organizationId` | string | Organization containing the trace. |
...
<ParameterTable>
  ...
  | `organizationId` | string | Optional organization. Defaults to the repository configuration or active login. |
...
The server also exposes `server_info`, which reports the installed Endform CLI version and MCP server command.
